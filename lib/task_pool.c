...
static void *worker(void *task)
{
	...
	for(;;){
		...
		/* do work */
		log1(info, "Server thread %d woken up\n", t->num);
		t->func(t->data);
...
static void task_create(struct task *t, int num)
{
	...
	log1(info, "Created thread: %d", (int)t->tid);
}

static void task_free(struct task *t)
{
	if(!t) return;
	log1(info, "Cancelling thread: %d", (int)t->tid);
	mutex_lock(&t->mutex);
	...
	if(thread_cancel(t->tid) != 0){	
		log1(info, "Couldn't cancel thread, but OK(%s)",
				strerror(errno));
	}
	while(pthread_mutex_destroy(&t->mutex) == EBUSY){
		usleep(1);
	}
	cond_destroy(&t->cond);
	free(t);
...
struct task_pool *task_pool_init(int max)
{		
	int i;
	struct task_pool *tp;
	bool e = false;
...
int task_pool_add(struct task_pool *tp, void *(*func)(void *), void *data)
{
	...
	for(i=0; i < tp->max; i++){
		...
		if(!t->busy){			
			...
			t->func = func;
			t->data = data;
			log1(info, "Adding work for thread: %d\n", t->num);
			cond_signal(&t->cond);
			mutex_unlock(&t->mutex);
			ret = 0;
			break;						
